...
Meteor.startup(() => {
  ...
  WebApp.connectHandlers.use(async (request, response, next) => {
    ...
    try {
      document = await WebAppInternals.getBoilerplate(
        request,
        WebApp.defaultArch
      );
      ...
      const applicationRef: ApplicationRef = appModuleRef.injector.get(
        ApplicationRef
      );
      ...
      const platformState: PlatformState = appModuleRef.injector.get(
        PlatformState
      );
